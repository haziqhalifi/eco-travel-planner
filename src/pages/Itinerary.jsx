...
import {Calendar, Clock, MapPin, Coffee, Compass, Utensils, Building} from 'lucide-react';
...
function Itinerary() {
  ...
  const handleNewActivity = (e) => {
    ...
    const dayIndex = updatedTrip.days.findIndex(day => day.date === newActivity.date);

    if (dayIndex >= 0) {
      updatedTrip.days[dayIndex].activities.push({...newActivity});
      updatedTrip.days[dayIndex].activities.sort((a, b) => a.time.localeCompare(b.time));
    } else {
        ...
        {!showNewTrip ? (
            <>
                ...
                <div className="">
                  ...
                  <div className='text-center text-forest'>
                    ...
                    <div className="text-muted text-center mt-2 d-flex align-items-center justify-content-center">
                      ...
                      <span className='text-forest mx-2'>{new Date(trip.startDate).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric'
              ...
              {trip.days.map((day, dayIndex) => (
                  <Card key={dayIndex} className="shadow-sm mb-3 border-0" style={{backgroundColor: "#F5F8F5"}}>
                    <Card.Header className=" py-3 border-bottom" style={{backgroundColor: "#F5F8F5"}}>
                      <div className="d-flex justify-content-between align-items-center">
                        <h5 className="mb-0 fw-semibold text-forest">{formatDate(day.date)}</h5>
                        <span className="text-muted">{getDayNumber(day.date, trip.startDate)}</span>
                      </div>
                    </Card.Header>
                    ...
                    <ListGroup variant="flush">
                      {day.activities.map((activity, index) => (
                          <ListGroup.Item key={index} className="border-0 py-3" style={{backgroundColor: "#F5F8F5"}}>
                            <div className="d-flex">

                              {/*time*/}
                              <div className='text-center pt-1'>
                                <p className="me-4 text-forest">
                                  {activity.time.substring(0, 2)}{activity.time.substring(2)} {parseInt(activity.time) >= 12 ? 'PM' : 'AM'}
                                </p>
                              </div>
                              ...
                              <div className="text-center me-3" style={{width: '50px'}}>
                                <div
                                    ...
                                    style={{width: '40px', height: '40px', backgroundColor: '#4A7856'}}>
                                  {getIcon(activity.icon)}
                                </div>
                              </div>

                              <div className="pt-2 d-flex flex-column text-start ">
                                <h6 className="mb-1 text-forest fw-semibold align-content-start">{activity.place}</h6>
                                {activity.imageUrl && (
                                    <img src={activity.imageUrl} className='mt-2 rounded-3' style={{height: '200px'}}/>
                                )}
                                {activity.details && (
                                    <div className="small text-forest d-flex ">{activity.details}</div>
                                )}
              ...
              <div className="text-center mt-4">
                <Button variant="outline-primary" onClick={() => setShowNewTrip(true)}>
                  Create New Trip
                </Button>
              </div>
            </>
              ...
              <Card.Body>
                <Form>
                  ...
                  <div className="d-flex gap-2">
                    <Button variant="secondary" onClick={() => setShowNewTrip(false)}>
                      Cancel
                    </Button>
